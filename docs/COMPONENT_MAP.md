...
* **Failure Modes & Degradation**:
  * *Static Placeholder Configuration & Failed Fetch Path*: The frontend ships with static placeholder configuration (`SUPABASE_URL = "https://YOUR_SUPABASE_PROJECT_ID.supabase.co"` and `SUPABASE_ANON_KEY = "YOUR_SUPABASE_ANON_KEY"`). Because the initialization check `SUPABASE_URL.includes("supabase.co")` (`dashboard/index.html:158`) is a simple substring match, it evaluates to `true` on the placeholder string and instantiates `supabaseClient`. When `refreshData()` executes (line 171), the `if (!supabaseClient)` guard does not fire; queries are dispatched against the unresolvable placeholder host. Because `@supabase/supabase-js` resolves failed network requests with `{ data: null, error }` rather than rejecting, the subsequent truthy guards (`if (jobs)`, `if (programmes)`, `if (outreach)`) evaluate to `false` and skip DOM rendering. The page remains indefinitely in its initial static state (stat counters display `'0'`, and tables show `"Loading programmes..."` / `"Loading outreach history..."`) without throwing uncaught exceptions or presenting UI error modals.
