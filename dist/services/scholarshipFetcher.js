...
exports.runScholarshipSourcingPipeline = runScholarshipSourcingPipeline;
 ...
 */
async function runScholarshipSourcingPipeline() {
    console.log('🔄 Running School & Scholarship Sourcing Sweep...');
    let progInserted = 0;
    let scholInserted = 0;
    for (const prog of SEED_PROGRAMMES) {
        const { data: insertedProg, error } = await supabase_1.supabase.from('programmes').upsert({
            name: prog.name,
            ...
            notes: prog.notes
        }, { onConflict: 'main_link' }).select().single();
        if (!error && insertedProg) {
            ...
            if (prog.is_fully_funded) {
                ...
                const { error: scholError } = await supabase_1.supabase.from('scholarships').upsert({
