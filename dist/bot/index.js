...
bot.command('jobs', async (ctx) => {
    ...
    try {
        let { data: jobs, error } = await supabase_1.supabase
            .from('jobs')
            .select('*')
            .eq('status', 'open')
        ...
        if (!jobs || jobs.length === 0 || shouldFetch) {
            ...
            await (0, jobFetcher_1.runJobSourcingPipeline)();
            const { data: freshJobs, error: freshErr } = await supabase_1.supabase
                .from('jobs')
                .select('*')
                .eq('status', 'open')
                .order('created_at', { ascending: false })
...
bot.command('sop', async (ctx) => {
    ...
    if (!schoolId) {
        return ctx.reply(`✍️ *Statement of Purpose (SOP) Commands*:\n\n` +
            ...
            `• \`/schools\` - View all tracked MSc programmes and their IDs`, { parse_mode: 'Markdown' });
    }
...
async function handleSaveSopSample(ctx, sampleText) {
    if (!sampleText) {
        return ctx.reply(`✍️ *Sample SOP Style Guide*\n\n` +
            ...
            `Example: \`/sop_sample Growing up in Lagos, Nigeria, witnessing digital infrastructure break...\``, { parse_mode: 'Markdown' });
    }
    try {
        const { data: profiles, error: selectErr } = await supabase_1.supabase.from('user_profile').select('*').limit(1);
        if (selectErr)
            throw selectErr;
        const profile = profiles && profiles.length > 0 ? profiles[0] : null;
        if (profile) {
            const existingParsed = profile.parsed_json || {};
            const { error: updateErr } = await supabase_1.supabase.from('user_profile').update({
            ...
            const { error: insertErr } = await supabase_1.supabase.from('user_profile').insert({
        ...
        ctx.reply(`✅ *Sample SOP Style Guide Saved!*\n\n` +
            `Gemini AI will now use your authentic personal voice, paragraph structure, and narrative cadence whenever generating Statement of Purpose drafts via \`/sop <school_id>\`.`, { parse_mode: 'Markdown' });
    }
    catch (err) {
