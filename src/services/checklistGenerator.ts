...
export interface ChecklistTask {
  description: string;
  bufferDaysBeforeDeadline: number;
 ...
 */
export async function generateScholarshipChecklist(programmeId: string): Promise<Task[]> {
  ...
  let { data: scholApp } = await supabase.from('scholarship_applications').select('id').eq('programme_id', programmeId).single();
  if (!scholApp) {
  ...
  let tasksToCreate: ChecklistTask[] = [];
...
`;
  ...
  for (const item of tasksToCreate) {
    ...
    if (!taskError && taskData) {
      insertedTasks.push(taskData as Task);
    }
  }
