 ...
 */
export async function generateSopForProgramme(programmeId: string): Promise<string> {
  const { data: programme, error } = await supabase.from('programmes').select('*').eq('id', programmeId).single();
  if (error || !programme) {
 ...
 */
async function saveSopDraft(programmeId: string, sopText: string) {
  const { data: existing } = await supabase.from('scholarship_applications').select('id').eq('programme_id', programmeId).single();

  if (existing) {
    await supabase.from('scholarship_applications').update({
      sop_draft: sopText
    }).eq('id', existing.id);
  } else {
