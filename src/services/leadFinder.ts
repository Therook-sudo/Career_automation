 ...
 */
export async function findOrProposeContact(company: string, roleFocus: string = 'DevOps'): Promise<Contact> {
  // Check if contact for company already exists
  const { data: existing } = await supabase.from('contacts').select('*').eq('company', company).limit(1).single();
